...
const EditJobs = () => {
  ...
  const deleteJob = async () => {
    try {
      ...
      const res = await axios.delete(`/api/job/deleteJob/${id}`, {
  ...
  const submitHandler = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`/api/job/updateJob/${id}`, input, {
        withCredentials: true,
      });
      ...
      <div className="w-full flex justify-center">
        <div className="bg-white mb-20 shadow-2xl max-w-5xl flex flex-col gap-4 rounded-lg p-4 mt-10 ">
          ...
          <div className="flex gap-7 justify-end">
            ...
            {loading ? (
              <Button className={"bg-blue-800 text-white cursor-not-allowed"}>
